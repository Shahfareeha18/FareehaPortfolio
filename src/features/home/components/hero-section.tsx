...
const HeroSection = () => {
  ...
  const nextImage = () => setCurrentImg((prev) => (prev + 1) % galleryImages.length);
  const prevImage = () => setCurrentImg((prev) => (prev - 1 + galleryImages.length) % galleryImages.length);
  ...
  return (
    <div className="relative pt-24 pb-14 text-center">
      ...
      {/* Cropped Image */}
      <div className="mx-auto w-full max-w-3xl cursor-pointer overflow-hidden rounded-xl" onClick={() => openGallery(0)}>
        <Image
          src={galleryImages[0]}
